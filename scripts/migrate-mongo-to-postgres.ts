/*
  ...
  - Requires MONGO_URI (source) and DATABASE_URL (target, Supabase/Postgres) in env
  - Reads collections: users, brandprofiles, posts, generationbatches
  - Inserts into Postgres via Prisma and maps old Mongo _id => new UUIDs
  - Usage (local):
    MONGO_URI="..." DATABASE_URL="..." npx ts-node ./scripts/migrate-mongo-to-postgres.ts

  Important: This script is intended as a starting point. Test on a copy of your DB!
*/

import { MongoClient } from 'mongodb'
import { PrismaClient } from '@prisma/client'
// Load environment variables (use require to avoid depending on ambient types)
...
async function run() {
  ...
  // Map from old Mongo _id to new UUID
  const idMap = { users: new Map<string, string>(), posts: new Map<string, string>(), batches: new Map<string, string>(), profiles: new Map<string, string>() }
